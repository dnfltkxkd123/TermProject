...
$(document).ready(function(){
	var myKey = "pwwBcPUBxiq1D7q8pTL4wA"; // sweet tracker에서 발급받은 자신의 키 넣는다./
                ...
                //stringify 메소드는 json 객체를 String 객체로 변환
                    ...
                    if(value.Code=='<?=$code?>'){
                      selected = 'selected';
	            			...
	            			myData += ('<option value='+value.Code+' '+selected+'>' +'key:'+key+', Code:'+value.Code+',Name:'+value.Name + '</option>');
                    //alert(value.Code);        				
            		});
        ...
        //
        var testCheck=<?=$invoice_number?>||null;
        var testCode='<?=$code?>'||null;
        if(testCheck  != null && testCode  != null){
          $(document).ready(function(){
            var t_code = testCode;
            var t_invoice = testCheck;
            $('#invoiceNumberText').val(t_invoice);
            //$("#tekbeCompnayList").val(testCode).prop("selected", true);
            //$('#tekbeCompnayList option[value='+testCode+']').attr('selected',true);
